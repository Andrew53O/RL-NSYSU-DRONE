#!/usr/bin/env python3
"""Train PPO for the Part 4 six-stage drone curriculum."""
...
MODEL_DIR = PART4_DIR / "models"
LOG_DIR = PART4_DIR / "logs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Part 4 drone curriculum PPO.")
...
def build_training_curve(
    ...
    png_path: Path,
    csv_path: Path,
    title: str = "Part 4 PPO Training Curve",
) -> bool:
    rewards: list[float] = []
...
def write_run_config(path: Path, args: argparse.Namespace, spec_name: str, run_name: str, model_dir: Path, log_dir: Path) -> None:
    config = {
        ...
        "notes": "Sonar observation fields are masked before Stage 5.",
    }
...
def main() -> None:
    ...
    try:
        ...
        print(f"Training Part 4 {spec.name}: {spec.description}")
        callbacks = [
        ...
        curve_title = f"Part 4 PPO Training Curve - Stage {args.stage} Variant {variant} {run_name}"
