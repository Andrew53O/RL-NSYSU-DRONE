#!/usr/bin/env python3
"""Part 4 Gymnasium environment for the expanded NSYSU drone PPO curriculum.

Part 4 keeps one fixed observation/action interface across all stages. Sonar
fields exist from Stage 1, but they are masked to safe constants until Stage 5
so early checkpoints can continue training in obstacle stages. Compared with
Part 3, Part 4 inserts a y-axis movement stage before combined navigation.
"""
...
STAGE_SPECS: dict[tuple[int, str], StageSpec] = {
    ...
    (3, "A"): StageSpec(
        ...
        description="fixed y target with stable x and altitude",
...
class DroneRosBridge(Node):
    ...
    def __init__(self, namespace: str = "/simple_drone") -> None:
        super().__init__("part4_drone_curriculum_env")
        ns = namespace.rstrip("/")
...
class DroneCurriculumEnv(gym.Env):
    """Six-stage Part 4 curriculum with masked sonar before obstacle stages."""
    ...
    def current_target(self) -> np.ndarray:
        if self.stage in (5, 6):
            return self._long_mission_local_target()
    ...
    def mission_goal(self) -> np.ndarray:
        if self.stage in (5, 6):
            ...
            return LONG_MISSION_FINAL_GOAL
        return self.targets[-1]
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        for attempt in range(3):
            ...
            if takeoff_ok and self.ros.pose is not None and self.ros.pose[2] >= self.min_altitude:
                break
            self.ros.get_logger().warning(f"Retrying Part 4 reset/takeoff {attempt + 1}/3")
    ...
    def step(self, action: np.ndarray):
        ...
        if self.stage in (5, 6) and self.previous_final_distance is not None:
        ...
        if self.sonar_enabled:
            ...
            if self.stage in (5, 6) and obstacle_max_risk > 0.2 and abs(float(info["vx"])) < 0.05:
                reward -= 0.2
    ...
    def _axis_progress_reward(self, delta: np.ndarray) -> float:
        if self.stage_spec.focus == "vertical":
            ...
            weights = np.array([4.0, 12.0, 6.0], dtype=np.float32)
        else:
    ...
    def _target_reached(self, x_error: float, y_error: float, z_error: float, distance: float) -> bool:
        if self.stage in (5, 6) and self.ros.pose is not None:
