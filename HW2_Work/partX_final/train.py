#!/usr/bin/env python3
"""Train PPO for the Part X final drone curriculum."""
...
MODEL_DIR = PART_DIR / "models"
LOG_DIR = PART_DIR / "logs"


class BestTrainingModelCallback(BaseCallback):
    """Save only the best moving-average checkpoint for final training."""

    def __init__(
        self,
        best_model_dir: Path,
        summary_mirror_path: Path,
        window: int = 20,
        verbose: int = 0,
    ) -> None:
        super().__init__(verbose=verbose)
        self.best_model_dir = best_model_dir
        self.summary_mirror_path = summary_mirror_path
    ...
    def _on_training_start(self) -> None:
        ...
        self.summary_mirror_path.parent.mkdir(parents=True, exist_ok=True)
        for stale_name in (
            "best_episode_model.zip",
            "best_success_model.zip",
            "best_precision_model.zip",
        ):
            stale_path = self.best_model_dir / stale_name
            if stale_path.exists():
                stale_path.unlink()
        with self.summary_path.open("w", newline="") as fp:
            writer = csv.DictWriter(fp, fieldnames=self.summary_fields)
            writer.writeheader()
        shutil.copy2(self.summary_path, self.summary_mirror_path)
                    ...
                    writer.writerow(
                        {
                            ...
                            "saved": "best_average_model",
                        }
                    )
                shutil.copy2(self.summary_path, self.summary_mirror_path)
                if self.verbose:
                    print(f"Saved best_average_model at episode {len(self.rewards)}")
        return True
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Part X final PPO drone curriculum.")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Seconds of ROS/Gazebo time to wait after each action. The final fork defaults to 0.05 for faster deadline runs.",
    )
    ...
    parser.add_argument(
        "--early-stop-plateau",
        dest="early_stop_plateau",
        action="store_true",
        default=True,
        help="Stop when the moving average reward no longer improves. Enabled by default.",
    )
    parser.add_argument(
        "--no-early-stop-plateau",
        dest="early_stop_plateau",
        action="store_false",
...
def build_training_curve(
    ...
    png_path: Path,
    csv_path: Path,
    title: str = "Part X Final PPO Training Curve",
) -> bool:
    rewards: list[float] = []
...
def write_run_config(path: Path, args: argparse.Namespace, spec_name: str, run_name: str, model_dir: Path, log_dir: Path) -> None:
    config = {
        ...
        "notes": (
            "Part X final fork: resume between stages from best_average_model.zip. "
            "Stage 4 target reaching uses anti-oscillation reward shaping and direct success."
        ),
    }
...
def main() -> None:
    ...
    try:
        ...
        print(f"Training Part X final {spec.name}: {spec.description}")
        callbacks = [
            BestTrainingModelCallback(
                best_model_dir=best_dir,
                summary_mirror_path=log_dir / "best_summary.csv",
        ...
        curve_title = (
            f"Part X Final PPO Training Curve - "
            f"Stage {args.stage} Variant {variant} {run_name}"
        )
        ...
        print(f"Saved best_average_model and best_summary.csv under: {best_dir}")
