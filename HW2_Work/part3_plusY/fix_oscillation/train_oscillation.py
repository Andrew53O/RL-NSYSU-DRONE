#!/usr/bin/env python3
"""Train PPO for the Part 3+Y anti-oscillation experiment."""
...
from drone_env_oscillation import DroneCurriculumEnv, get_stage_spec, normalize_variant
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Part 3+Y PPO with anti-oscillation reward shaping.")
...
def build_training_curve(
    ...
    png_path: Path,
    csv_path: Path,
    title: str = "Part 3+Y Anti-Oscillation PPO Training Curve",
) -> bool:
    rewards: list[float] = []
...
def write_run_config(path: Path, args: argparse.Namespace, spec_name: str, run_name: str, model_dir: Path, log_dir: Path) -> None:
    config = {
        ...
        "notes": (
            ...
            "and near-target circling receives extra velocity/tangential penalties."
        ),
    }
...
def main() -> None:
    ...
    try:
        ...
        print(f"Training Part 3+Y anti-oscillation {spec.name}: {spec.description}")
        callbacks = [
        ...
        curve_title = (
            f"Part 3+Y Anti-Oscillation PPO Training Curve - "
