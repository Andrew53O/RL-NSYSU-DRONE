#!/usr/bin/env python3
"""Train PPO for the Part 3+Y seven-stage drone curriculum."""
...
def build_training_curve(
    ...
    png_path: Path,
    csv_path: Path,
    title: str = "Part 3+Y PPO Training Curve",
) -> bool:
    rewards: list[float] = []
...
def main() -> None:
    ...
    try:
        ...
        print(f"Training Part 3+Y {spec.name}: {spec.description}")
        callbacks = [
        ...
        curve_title = f"Part 3+Y PPO Training Curve - Stage {args.stage} Variant {variant} {run_name}"
